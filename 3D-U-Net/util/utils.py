...
'''def checkpoint_restore(model, exp_path, exp_name, use_cuda=True, epoch=0, dist=False, f=''):
    ...
    if not f:
        if epoch > 0:
            ...
            f = sorted(glob.glob(os.path.join(exp_path, exp_name + '-*_weight.pth')))
            if len(f) > 0:
                f = f[-1]
                epoch = int(f[len(exp_path) + len(exp_name) + 2 : -4].split('_')[0])

    if len(f) > 0:
        logger.info('Restore from ' + f)
        checkpoint = torch.load(f)
        print (f)
        #print (f[:-11]+'_feat.pth')
        feat=torch.load(f[:-11]+'_feat.pth')
        for k, v in checkpoint.items():
            if 'module.' in k:
                checkpoint = {k[len('module.'):]: v for k, v in checkpoint.items()}
            break
